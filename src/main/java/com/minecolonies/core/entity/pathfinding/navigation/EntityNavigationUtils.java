...
public class EntityNavigationUtils
{
    ...
    public static BlockPos surfaceAt(Level level, int x, int z)
    {
        ...
        BlockPos.MutableBlockPos localSearchPos = pos.mutable();
        for (int i = 0; i < 6 && localSearchPos.getY() > minY; i++)
        {
            localSearchPos.move(0, -1, 0);
            if (isStandable(level, localSearchPos)) 
            {
                return localSearchPos.immutable();
            }
        }

        // If that failed, try stepping upward (e.g., if we landed inside a fence/crop)
        localSearchPos.set(pos);
        for (int i = 0; i < 6 && localSearchPos.getY() < maxY; i++)
        {
            localSearchPos.move(0, 1, 0);
            if (isStandable(level, localSearchPos)) 
            {
     ...
     * Example use case: Colony patrolling (which cannot enter buildings)
    ...
    public static BlockPos closestOutsideCornerofBuilding(@Nonnull IBuilding building, @Nonnull BlockPos referencePos)
    {
        ...
        {
            if (!EntityNavigationUtils.isStandable(level, p))
            {
                continue;
            }
